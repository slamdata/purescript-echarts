...
instance decodeSeries ∷ DecodeJson Series where
  decodeJson json = do
    ...
    case ty of
      "line" →
        LineSeries <$> ({common: u, lineSeries: _} <$> decodeLineRec obj)
      "bar" →
        BarSeries <$> ({common: u, barSeries: _} <$> decodeBarRec obj)
      "scatter" →
        ScatterSeries <$> ({common:u, scatterSeries: _} <$> decodeScatterSeriesRec obj)
      "k" →
        CandlestickSeries <$> ({common:u, candlestickSeries: _} <$> decodeCandleStickSeries obj)
      "pie" →
        PieSeries <$> ({common: u, pieSeries: _} <$> decodePieSeriesRec obj)
      "radar" →
        RadarSeries <$> ({common: u, radarSeries: _} <$> decodeRadarSeriesRec obj)
      "chord" →
        ChordSeries <$> ({common: u, chordSeries: _} <$> decodeChordSeriesRec obj)
      "force" →
        ForceSeries <$> ({common: u, forceSeries: _} <$> decodeForceSeriesRec obj)
      "map" →
        MapSeries <$> ({common: u, mapSeries: _} <$> decodeMapSeriesRec obj)
      "gauge" →
        GaugeSeries <$> ({common: u, gaugeSeries: _} <$> decodeGaugeSeriesRec obj)
      "funnel" →
        FunnelSeries <$> ({common: u, funnelSeries: _} <$> decodeFunnelSeriesRec obj)
      "eventRiver" →
        EventRiverSeries <$> ({common: u, eventRiverSeries: _} <$> decodeEventRiverSeriesRec obj)
      _ → Left "Invalid Series"


foreign import setSeriesImpl ∷ ∀ e. Fn3 (Array Json) Boolean EChart (Eff e EChart)

setSeries ∷ ∀ e. Array Series → Boolean → EChart → Eff e EChart
